...
export function renderAppendix(doc, model, _def) {
    ...
    if (sec) {
        ...
        doc.moveDown(0.2);
        doc.font('Helvetica').fontSize(9).fillColor(theme.colors.slate700).text(`This document was issued by ${model.branding.companyName} and is protected by SHA-256: ${sec.sha256Hash}`);
        doc.moveDown(0.6);
    }
    // Attachments listing

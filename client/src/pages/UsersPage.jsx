...
const ROLE_CONFIG = {
  admin: { class: 'text-violet-400 bg-violet-500/10 border-violet-500/20' },
  manager: { class: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
  tenant: { class: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }}
        className="w-full max-w-md rounded-2xl border border-violet-500/20 bg-[#0a0f1e]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="text-lg font-black text-white">{isEdit ? 'Edit User' : 'Create User'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {['tenant', 'manager', 'admin'].map(r => (
                <button key={r} type="button" disabled={isEdit} onClick={() => set('role', r)}
                  className={cn('flex-1 py-2 rounded-xl border text-xs font-black capitalize transition-all', form.role === r
                    ? ROLE_CONFIG[r]?.class
                    : 'border-white/10 text-white/25 hover:border-white/20 disabled:opacity-40')}>
                  {r}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-3 rounded-xl border border-white/10 text-white/40 hover:text-white font-bold transition-all">Cancel</button>
            <button type="submit" disabled={loading}
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white font-black hover:opacity-90 transition-all disabled:opacity-50">
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} required={required} disabled={disabled}
        className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-violet-500/50 transition-all disabled:opacity-40 disabled:cursor-not-allowed" />
    </div>
  );
}
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.25em] text-violet-400">Administration</p>
          </div>
          <h1 className="text-3xl font-black text-white">Users <span className="text-white/20 font-bold text-lg">({total})</span></h1>
        </div>
        <button onClick={() => setModal('add')}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white font-black text-sm hover:opacity-90 transition-all shadow-lg shadow-violet-500/20">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder-white/20 focus:outline-none focus:border-violet-500/40 transition-all" />
        </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="text-center py-16 text-white/30">No users found</td></tr>
            ) : users.map((u, i) => {
                ...
                <motion.tr key={u._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className={cn('w-9 h-9 rounded-xl bg-gradient-to-br flex items-center justify-center text-white font-black text-sm flex-shrink-0',
                        u.role === 'admin' ? 'from-violet-600 to-purple-700' : u.role === 'manager' ? 'from-blue-600 to-cyan-700' : 'from-emerald-600 to-teal-700')}>
                  ...
                  <td className="px-5 py-3.5 text-sm text-white/40">{u.phone || '—'}</td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5">
                    <div className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-bold',
                      u.isActive ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-white/30 bg-white/5 border-white/10')}>

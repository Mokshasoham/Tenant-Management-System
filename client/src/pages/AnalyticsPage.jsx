...
function BarChart({ data, valueKey, labelKey, color = '#3b82f6', height = 140 }) {
    if (!data?.length) return <div className="text-center py-8 text-muted-foreground/30 text-sm">No data</div>;
            ...
            {data.map((d, i) => {
                ...
                return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1 group" title={`${d[labelKey]}: ₹${(d[valueKey] || 0).toLocaleString('en-IN')}`}>
                        <div className="w-full rounded-t-md transition-all relative" style={{ height: `${pct}%`, minHeight: 4, background: color, opacity: 0.8 }}>
                            <div className="absolute -top-7 left-1/2 -translate-x-1/2 hidden group-hover:block text-[9px] text-white/90 font-bold whitespace-nowrap bg-black/80 px-1 py-0.5 rounded shadow-xl">
                                ₹{(d[valueKey] || 0).toLocaleString('en-IN')}
                            </div>
                        </div>
                        <span className="text-[8px] text-muted-foreground/40 dark:text-white/25 truncate w-full text-center">{d[labelKey]}</span>
                    </div>
                );
            })}
        </div>
    ...
    return (
        <div className="relative" style={{ width: size, height: size }}>
            <svg width={size} height={size} viewBox="0 0 100 100" className="-rotate-90">
                <circle cx="50" cy="50" r={r} fill="none" stroke="currentColor" className="text-muted/10 dark:text-white/5" strokeWidth="10" />
...
function CollectionChart({ data }) {
    if (!data?.length) return <div className="text-center py-4 text-muted-foreground/30 text-sm">No data</div>;
    const max = 100;
    return (
        <div className="space-y-2">
            {data.map((d, i) => (
                <div key={i} className="flex items-center gap-3">
                    <span className="text-[10px] text-muted-foreground/40 w-12 text-right flex-shrink-0">{d.month}</span>
                    <div className="flex-1 h-2 rounded-full bg-muted border border-border/50 overflow-hidden">
                        <motion.div
                    ...
                    <span className="text-xs font-bold text-muted-foreground/70 w-10 text-right flex-shrink-0">{d.rate}%</span>
                </div>
            ))}
        </div>
    );
...
export default function AnalyticsPage() {
    ...
    if (loading) return <div className="flex items-center justify-center h-64 text-muted-foreground/30">Loading analytics...</div>;
    ...
    return (
        <div className="space-y-6 pb-8">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <select value={months} onChange={e => setMonths(Number(e.target.value))}
                        className="px-3 py-2 rounded-xl bg-muted border border-border text-foreground text-sm focus:outline-none appearance-none cursor-pointer">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 p-5 rounded-2xl border border-border bg-card shadow-sm transition-colors">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-black text-foreground">Revenue Over Time</h3>
                        <p className="text-xs text-muted-foreground/40 font-bold">₹{revenue.reduce((a, d) => a + d.value, 0).toLocaleString('en-IN')} total</p>
                    </div>
                            ...
                            <div className="w-full space-y-2">
                                {[
                                    ...
                                    <div key={k} className="flex items-center justify-between text-xs">
                                        ...
                                        <span className="font-bold text-foreground/70">{occupancy[k]}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ) : <div className="text-center text-muted-foreground/20 py-8">No data</div>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="p-5 rounded-2xl border border-border bg-card shadow-sm transition-colors">
                    <h3 className="font-black text-foreground mb-4">Top Revenue Properties</h3>
                    {topProps.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground/30 text-sm">No data yet</div>
                    ) : (
                            ...
                            {topProps.map((p, i) => (
                                <div key={p._id} className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-foreground/80 truncate">{p.propertyName || 'Unknown'}</p>
                                        <p className="text-[10px] text-muted-foreground/30">{p.paymentCount} payments</p>
                                    </div>
                                    <p className="text-sm font-black text-foreground">₹{p.totalRevenue?.toLocaleString('en-IN')}</p>
                                </div>
                            ))}
                        </div>
                    )}

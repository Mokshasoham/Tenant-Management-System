    ...
    >
      ...
      >
        <div className="sticky top-0 flex items-center justify-between px-6 py-4 border-b border-border bg-card/95 backdrop-blur-sm z-10 transition-colors">
          <h2 className="text-lg font-black text-foreground">{property ? 'Edit Property' : 'Add Property'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">State / Region *</label>
              {form.country === 'India' ? (
                <select value={form.state} onChange={e => set('state', e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl bg-muted border border-border text-foreground text-sm focus:outline-none focus:border-primary/50 transition-all appearance-none">
                  <option value="" className="bg-card">Select State</option>
                  {INDIA_STATES.map(s => <option key={s} value={s} className="bg-card">{s}</option>)}
                </select>
              ) : (
          ...
          <div className="grid grid-cols-2 gap-3">
            <SelectField label="Booking Type" value={form.bookingType} onChange={v => set('bookingType', v)}
              options={['paid', 'free']} />

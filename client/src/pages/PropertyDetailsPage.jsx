                ...
                {isManager ? (
                    ...
                    <div className="space-y-6">
                        ...
                        {(() => {
                            ...
                            const managerName = mgr?.name || (mgr?.firstName ? `${mgr.firstName} ${mgr?.lastName || ''}`.trim() : null);
                            const hasManager = Boolean(mgr && (managerName || mgr.email || mgr.phone || mgr.phoneNumber));
                            const managerInitials = (mgr?.firstName?.[0] || mgr?.name?.[0] || 'M') + (mgr?.lastName?.[0] || '');
                            ...
                            const isChatAvailable = Boolean(hasManager && mgr?._id && (hasActiveBooking || hasActiveLeaseOnProp));
                                    ...
                                    <div className="flex items-center gap-4">
                                        {mgr?.avatar ? (
                                            ...
                                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 dark:from-emerald-500 dark:to-cyan-600 flex items-center justify-center text-xl font-black text-white shadow-lg shadow-emerald-500/20">
                                                {hasManager ? managerInitials.toUpperCase() : '?'}
                                            </div>
                                        )}
                                        <div className="space-y-0.5 min-w-0 flex-1">
                                            <h3 className="text-base sm:text-lg font-black text-foreground truncate">
                                                {hasManager ? managerName || mgr.email : 'Manager not assigned'}
                                            </h3>
                                            <p className="text-xs text-muted-foreground font-medium">
                                                {hasManager ? 'Dedicated Property Manager' : 'No manager currently assigned'}
                                            </p>
                                        </div>
                                    </div>
                                            ...
                                            <div className="flex items-center justify-between p-3 rounded-2xl bg-muted/40 border border-border/60 text-xs">
                                                <div className="flex items-center gap-2.5 min-w-0">
                                                    ...
                                                    <span className="font-bold text-foreground truncate">
                                                        {mgr.phone || mgr.phoneNumber || 'Phone not available'}
                                                    </span>
                                                </div>
                                                {(mgr.phone || mgr.phoneNumber) && (
                                                    <a
                                                        href={`tel:${mgr.phone || mgr.phoneNumber}`}

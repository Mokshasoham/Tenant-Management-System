...
export function NotificationFilterBar({
    ...
    return (
        <div className="space-y-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800/80 backdrop-blur-md">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                ...
                <div className="relative flex-1 max-w-md">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all"
                    />
                    {search && (
                        <button
                ...
                <div className="flex items-center space-x-3 shrink-0 flex-wrap gap-y-2">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="bg-slate-950/80 border border-slate-800 text-xs font-medium text-slate-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 cursor-pointer"
                        >
                    ...
                    {/* Unread Only Switch */}
                    <label className="inline-flex items-center cursor-pointer space-x-2 bg-slate-950/80 border border-slate-800 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-300 hover:bg-slate-850 transition-colors">
                        <input
                            type="checkbox"
                            checked={unreadOnly}
                    ...
                    return (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all duration-200 border ${
                                isActive
                                    ? 'bg-indigo-600/90 text-white border-indigo-500 shadow-md shadow-indigo-950/40'
            ...
            {/* Bottom Toolbar: Bulk Selection Bar (When Items Selected or Available) */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-800/60 text-xs">
                <div className="flex items-center space-x-3 text-slate-400">
                    <label className="inline-flex items-center space-x-2 cursor-pointer">
                        ...
                        />
                        <span className="font-medium text-slate-300">Select All Page</span>
                    </label>
                    {selectedCount > 0 && (
                    ...
                    {selectedCount > 0 ? (
                        <>
                            ...
                            <button
                                ...
                                className="px-2.5 py-1.5 rounded-lg text-slate-400 hover:text-slate-200 transition-colors"
                            >
                                Clear Selection
                            </button>
                        </>

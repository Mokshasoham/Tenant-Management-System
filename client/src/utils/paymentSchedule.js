...
export function calculateNextPaymentDue(lease, leasePayments = [], currentDate = new Date()) {
  ...
  if (lease.nextPaymentDueAt && lease.nextPaymentSchedule) {
    ...
    return {
      ...
      amount: s.totalDue ?? s.amount ?? lease.nextPaymentAmount ?? lease.rentAmount,
      rentAmount: s.rentAmount ?? lease.rentAmount,
      lateFeePerDay: s.lateFeePerDay ?? (lease.lateFeePerDay || 100),
      daysLate: s.daysLate ?? 0,
      daysLeft: s.daysLeft ?? 0,
      lateFee: s.lateFee ?? 0,
      totalDue: s.totalDue ?? s.amount ?? lease.nextPaymentAmount ?? lease.rentAmount,
      status: s.status || lease.nextPaymentStatus || 'upcoming',
      isOverdue: s.isOverdue ?? (s.status === 'overdue'),
      isDueToday: s.isDueToday ?? (s.status === 'due'),
      isUpcoming: s.isUpcoming ?? (s.status === 'upcoming' || s.status === 'scheduled'),
      ...
      billingPeriodStart: s.billingPeriodStart || null,
      billingPeriodEnd: s.billingPeriodEnd || null,
